...
(deftest simple-component-spec-test
  (testing "Basic sequence matching logic for field only components is working correctly"
    (is (matching-component? [{:tag :1, :value "some-value"}] {:ordering [{:tag      :1
                                                                           :required true
                                                                           :type     :field}]}))
    (is (matching-component? [{:tag :1, :value "some-value"}] {:ordering [{:tag      :1
                                                                           :required true
                                                                           :type     :field}]}))
    (is (not (matching-component? [{:tag :1, :value "some-value"}] {:ordering [{:tag      :2
                                                                                :required false
                                                                                :type     :field}]})))
    (is (not (matching-component? [{:tag :1, :value "some-value"}] {:ordering [{:tag      :2
                                                                                :required true
                                                                                :type     :field}]})))
    (is (matching-component? [] {:ordering []}))
    (is (not (matching-component? [{:tag :x, :value "some-value"}] {:ordering []})))
    ...
    (is (not (matching-component? [{:tag :1, :value "some-value"}
                                   ...
                                   {:tag :4, :value "some-value"}] {:ordering [{:tag      :1
                                                                               ...
                                                                               {:tag      :3
                                                                                :required true
                                                                                :type     :field}]})))
    (is (not (matching-component? [{:tag :X, :value "some-value"}] {:ordering [{:tag      :1
                                                                                :required true
                                                                                :type     :field}
                                                                               {:tag      :2
                                                                                :required true
...
(deftest basic-group-spec-test
  (testing "Num-in-group repetitions are correctly applied and simple groups, which only contain fields are matched correctly"
    ...
    (is (not (matching-component? [{:tag :146 :value 0}] {:ordering [{:type     :group,
                                                                      :required true,
                                                                      :name     "TestGroupA",
                                                                      :ordering
                                                                                [{:tag :146, :required true, :type :field}
