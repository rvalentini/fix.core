...
(deftest simple-component-spec-test
  (testing "Basic sequence matching logic for field only components is working correctly"
    (is (matching-seqs? [{:tag :1, :value "some-value"}] {:ordering [{:tag      :1
                                                                      :required true
                                                                      :type     :field}]}))
    (is (matching-seqs? [{:tag :1, :value "some-value"}] {:ordering [{:tag      :1
                                                                      :required true
                                                                      :type     :field}]}))
    (is (not (matching-seqs? [{:tag :1, :value "some-value"}] {:ordering [{:tag      :2
                                                                           :required false
                                                                           :type     :field}]})))
    (is (not (matching-seqs? [{:tag :1, :value "some-value"}] {:ordering [{:tag      :2
                                                                           :required true
    ...
    (is (not (matching-seqs? [{:tag :x, :value "some-value"}] {:ordering []})))
    (is (not (matching-seqs? [{:tag :1, :value "some-value"}
                              {:tag :3, :value "some-value"}] {:ordering [{:tag      :x
                                                                           :required true
                                                                           :type     :field}
                                                                          {:tag      :1
                                                                           :required true
                                                                          ...
                                                                          {:tag      :3
                                                                           :required true
                                                                           :type     :field}]})))
    (is (matching-seqs? [{:tag :1, :value "some-value"}
                         {:tag :3, :value "some-value"}] {:ordering [{:tag      :x
                                                                      :required false
                                                                      :type     :field}
                                                                     {:tag      :1
  ...
  (testing "Tags that are not contained in the definition/seq will fail the matching"
    (is (not (matching-seqs? [{:tag :1, :value "some-value"}
                              ...
                              {:tag :4, :value "some-value"}] {:ordering [{:tag      :1
                                                                           :required true
                                                                           :type     :field}
                                                                          {:tag      :2
                                                                           :required true
                                                                          ...
                                                                          {:tag      :3
                                                                           :required true
                                                                           :type     :field}]})))
    (is (not (matching-seqs? [{:tag :X, :value "some-value"}] {:ordering [{:tag      :1
                                                                           :required true
                                                                           :type     :field}
                                                                          {:tag      :2
                                                                           :required true
  ...
  (testing "Potentially successful matching for given tag sequence will still fail, when there are mandatory tags missing"
    ...
    (is (not (matching-seqs? [] {:ordering [{:tag      :b
                                             :required true
                                             :type     :field}]})))
    (is (not (matching-seqs? [{:tag :a, :value "some-value"}
                              {:tag :b, :value "some-value"}
                              {:tag :c, :value "some-value"}] {:ordering [{:tag      :a
                                                                           :required false
                                                                           :type     :field}
                                                                          {:tag      :b
                                                                           :required false
                                                                          ...
                                                                          {:tag      :d
                                                                           :required true
                                                                           :type     :field}]})))
    (is (matching-seqs? [{:tag :a, :value "some-value"}
                         {:tag :b, :value "some-value"}
                         {:tag :c, :value "some-value"}] {:ordering [{:tag      :a
                                                                      :required false
                                                                      :type     :field}
...
(deftest nested-component-spec-test
  (testing "Complex structures containing nested components are matched correctly for required and non-required fields"
    (is (matching-seqs? [{:tag :a, :value "some-value"}
                         ...
                         {:tag :c3, :value "some-value"}] {:ordering
                                                           [{:tag      :a
                                                             :required true
                                                             :type     :field}
                                                            {:tag      :b
    ...
    (is (matching-seqs? [{:tag :a, :value "some-value"}
                         ...
                         {:tag :c23, :value "some-value"}] {:ordering
                                                            [{:tag      :a
                                                              :required true
                                                              :type     :field}
                                                             {:tag      :b
    ...
    (is (matching-seqs? [{:tag :a, :value "some-value"}
                         ...
                         {:tag :c4, :value "some-value"}] {:ordering
                                                           [{:tag      :a
                                                             :required true
                                                             :type     :field}
                                                            {:tag      :b
...
(deftest basic-group-spec-test
  (testing "Num-in-group repetitions are correctly applied and simple groups, which only contain fields are matched correctly"
    ...
    (is (matching-seqs? [{:tag :146 :value 2}
                         {:tag :63 :value "some-value"}
                         {:tag :1617 :value "some-value"}
                         {:tag :63 :value "some-value"}
                         {:tag :1617 :value "some-value"}] {:ordering
                                                            [{:type     :group,
                                                              :required true,
                                                              :name     "TestGroupA",
                                                              :ordering
    ...
    (is (not (matching-seqs? [{:tag :146 :value 2}
                              {:tag :63 :value "some-value"}
                              {:tag :1617 :value "some-value"}
                              {:tag :63 :value "some-value"}
                              {:tag :534 :value "this-does-not-fit"}] {:ordering
                                                                       [{:type     :group,
                                                                         :required true,
                                                                         :name     "TestGroupA",
                                                                         :ordering
    ...
    (is (not (matching-seqs? [{:tag :146 :value 0}] {:ordering [{:type     :group,
                                                                 :required true,
                                                                 :name     "TestGroupA",
                                                                 :ordering
                                                                           [{:tag :146, :required true, :type :field}
...
(deftest complex-nested-group-spec-test
  (testing "Groups containing nested components, which also contain groups a matched correctly"
    ...
    (is (matching-seqs? [{:tag :889 :value "some-value"}
                         ...
                         {:tag :c2 :value "some-value"}
                         {:tag :542 :value "some-value"}
                         {:tag :891 :value "some-value"}] {:ordering
    ...
    (is (not (matching-seqs? [{:tag :889 :value "some-value"}
                              {:tag :890 :value "some-value"}
                              ;TestGroupA
                              {:tag :146 :value 2}
                              ;TestGroupA - 1st repetition
